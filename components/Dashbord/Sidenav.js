    ...
    return (
        <div className="h-full w-full flex flex-col gap-y-4 relative">
            ...
            <div className="w-full flex flex-col items-center gap-y-3">
                ...
                <Image className="w-full h-full rounded-full" width={100} height={100} src={session.user.image} alt="this is an image"/>
                </div>

                <div>
                  <p className="font-medium text-lg">{session.user.name}</p>
                  <p className="font-normal text-sm opacity-30">{session.user.email}</p>
