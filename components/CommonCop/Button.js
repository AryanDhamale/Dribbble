...
function Button({ text, width, link, round }) {
    return (
        <>
            {
                link == 'no-' ? <button className={`bg-[#872bff] active:bg-[#872bffd4] font-medium cursor-pointer drop-shadow-lg text-white w-[${width}] rounded-${round} block mx-auto my-2.5 py-3`}>{text}</button> : <Link href={link}>
